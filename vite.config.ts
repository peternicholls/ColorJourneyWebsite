...
export default ({ mode }: { mode: string }) => {
  ...
  return defineConfig({
    ...
    optimizeDeps: {
      ...
      exclude: ["agents", "emsdk"], // Exclude agents package from pre-bundling due to Node.js dependencies, exclude emsdk (build toolchain)
      entries: ['src/**/*.{js,jsx,ts,tsx}'], // Only scan src directory, not emsdk
      force: true,
    },
    assetsInclude: ['**/*.wasm'],
    define: {

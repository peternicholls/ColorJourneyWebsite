      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between text-sm p-3 bg-muted rounded-md">
            <span className="text-muted-foreground">Min. Perceptual Distance (ΔE)</span>
            {isLoading ? <Skeleton className="h-5 w-16" /> : <span className="font-mono">{result?.diagnostics.minDeltaE.toFixed(3) ?? 'N/A'}</span>}
          </div>
          <div className="flex items-center justify-between text-sm p-3 bg-muted rounded-md">
            <span className="text-muted-foreground">Contrast Violations</span>
            {isLoading ? <Skeleton className="h-5 w-10" /> : <span className="font-mono">{result?.diagnostics.contrastViolations ?? 'N/A'}</span>}
          </div>
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            <Button
              ...
              onClick={() => handleCopy(exportToCssVariables(result!.palette), 'CSS Variables')}
            >
            ...
            <Button
              ...
              variant="outline"
              disabled={isLoading || !result}
              onClick={() => handleCopy(exportToJson(result!), 'JSON')}
            >
